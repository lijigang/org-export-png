...
- *标题入卡片*：导出整个文件时，=#+title:= / =#+subtitle:= 会变成有设计感的卡片标题区。
...
没有选区时，导出当前 subtree 或整个 buffer。整个 buffer 里若有 =#+title:= / =#+subtitle:=，会自动渲染为卡片顶部标题区。PNG 默认写到 =~/Downloads/= 并自动打开。
...
| =org-export-png-with-title= | =t= | 是否把 =#+title:= / =#+subtitle:= 渲染成卡片标题区 |
...
- =#+title:= / =#+subtitle:= 会在 HTML 片段外额外生成标题区，因为 Org 的 body-only 导出不会把文档标题放进正文；
